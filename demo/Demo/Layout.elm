...
picker hues disabled shade current f =
    hues
        ...
        |> List.map
            (\hue ->
                Options.styled_ div
                    [ when (Color.background (Color.color hue shade)) (disabled /= Just hue)
                    , when (Color.background (Color.color Color.Grey Color.S200)) (disabled == Just hue)
                    , css "width" "56px"
                    , css "height" "56px"
                    , css "margin" "2pt"
                    , css "line-height" "56px"
                    , css "flex-shrink" "0"
                    , when Elevation.e8 (current == hue)
                    , css "transition" "box-shadow 300ms ease-in-out 0s, background-color 300ms ease-in-out 0s"
                    , when (css "cursor" "pointer") (disabled /= Just hue)
                    ]
